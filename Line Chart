...
line1, = ax1.plot(x1, data1, label='mAP w/ Adversarial', color='#FF9500', linestyle='-', marker='o', markersize=5,
                  markevery=1, linewidth=2, clip_on=False)
line2, = ax1.plot(x1, data2, label='mAP w/o Adversarial', color='#D5558B', linestyle='--', marker='^', markersize=5,
                  markevery=1, linewidth=2, clip_on=False)
ax1.fill_between(x1, np.array(data1) - std_multiplier * np.std(data1), np.array(data1) + std_multiplier * np.std(data1),
                 color='#FF9500', alpha=0.2)
ax1.fill_between(x1, np.array(data2) - std_multiplier * np.std(data2), np.array(data2) + std_multiplier * np.std(data2),
                 color='#D5558B', alpha=0.2)
...
ax2.fill_between(x2, np.array(data3) - std_multiplier * np.std(data3), np.array(data3) + std_multiplier * np.std(data3),
                 color='#00B8DA', alpha=0.2)
ax2.fill_between(x2, np.array(data4) - std_multiplier * np.std(data4), np.array(data4) + std_multiplier * np.std(data4),
                 color='#8D5242', alpha=0.2)
ax2.set_ylabel('Loss', color='black', fontsize=14)"""#这段代码是用来画line3,line4
# Add title and labels
fig.text(0.5, 0.04, 'Epochs', ha='center', fontsize=14)
plt.title('Impact of Adversarial Learning')#用来增加图像标题

# Create a combined legend
lines = [line1, line2]#去掉 line3,line4
labels = [line.get_label() for line in lines]
plt.legend(lines, labels, fontsize=16, loc='right')#这里负责绘画图例
